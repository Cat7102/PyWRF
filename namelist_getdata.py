...
timezone = 8  # 设置时区
inheight = True  # 是否沿着高度输出数据，需要先确保数据存在高度，将启用heightrange与timenum
intime = False  # 是否沿着时间输出数据，将启用timerange与heightnum
inall = False  # 同时输出所有高度与时间的数据，将同时启用heightrange和timerange
heightnum = 2  # 如果数据存在高度，并且仅沿时间输出，输出数据的高度层，高度编号可以通过read_ncfile_height获得
heightrange = np.arange(0, 44, 1)  # 仅在inheight为True时生效，高度的层数范围，高度编号可以通过read_ncfile_height获得
